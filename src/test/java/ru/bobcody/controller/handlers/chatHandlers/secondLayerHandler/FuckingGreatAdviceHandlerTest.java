...
class FuckingGreatAdviceHandlerTest extends AbstractSpringBootStarterTest {
    @Mock //эдвайсер работает с внешним апи которое может в любой момент отвалиться поэтому мок
    FuckingGreatAdvice adviser = mock(FuckingGreatAdvice.class);
    @Autowired
    ...
    void init() throws IOException {
        when(adviser.getAdvice()).thenReturn("советую не обкакаться!");
        fuckingGreatAdviceHandler.setFuckingGreatAdvice(adviser);
    }
